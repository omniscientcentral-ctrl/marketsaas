...
CREATE OR REPLACE FUNCTION public.create_customer_with_initial_debt(p_name text, p_last_name text DEFAULT NULL::text, p_document text DEFAULT NULL::text, p_rut text DEFAULT NULL::text, p_phone text DEFAULT NULL::text, p_address text DEFAULT NULL::text, p_credit_limit numeric DEFAULT 0, p_initial_debt numeric DEFAULT 0, p_notes text DEFAULT NULL::text, p_status text DEFAULT 'active'::text, p_cashier_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_initial_debt > 0 THEN
    ...
    VALUES (v_customer_id, v_full_name, p_phone, v_sale_id, p_initial_debt, p_initial_debt, 0, 'pending');
  END IF;
