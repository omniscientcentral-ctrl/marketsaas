...
const CashRegistersManagement = () => {
  const { user, loading, activeRole, userRoles } = useAuth();
  ...
  const handleToggleActive = async (register: CashRegister) => {
    try {
      // Si se intenta desactivar, verificar si tiene sesión abierta
  ...
  if (loading || !activeRole || loadingData) {
    return (
...
export default CashRegistersManagement;
